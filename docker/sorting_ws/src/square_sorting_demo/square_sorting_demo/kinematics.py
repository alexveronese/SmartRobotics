"""Small serial-chain kinematics helper for the Chapter 9 Panda URDF."""
...
def _transform(xyz, rpy):
    result = np.eye(4)
    result[:3, :3] = (
        _rotation_z(rpy[2]) @ _rotation_y(rpy[1]) @ _rotation_x(rpy[0])
    )
    result[:3, 3] = xyz
    return result
...
class SerialChain:
    ...
    def __init__(self, robot_description, base_link="world", tip_link="hand_tcp"):
    ...
    def _find_chain(cls, graph, current, target, visited):
        ...
        for joint in graph.get(current, []):
            result = cls._find_chain(
                graph, joint.child, target, visited + [joint]
            )
            if result:
                return result
        return []
    ...
    def forward(self, positions, with_jacobian=False):
        ...
        if positions.shape != (len(self.active_joints),):
            raise ValueError(
                f"Expected {len(self.active_joints)} joints, got {positions.shape}"
            )
        ...
        for index, joint in enumerate(self.active_joints):
            if joint.joint_type in ("revolute", "continuous"):
                jacobian[:3, index] = np.cross(
                    axes[index], end_position - origins[index]
                )
    ...
    def inverse(
        ...
        """Solve pose IK with damped least squares and a small posture bias."""
        target_position = np.asarray(target_position, dtype=float)
        target_rotation = np.asarray(target_rotation, dtype=float)
        seed = np.asarray(seed, dtype=float)
        preferred = seed.copy() if preferred is None else np.asarray(preferred)
        margin = 1e-4
        ...
        for candidate in candidates:
            ...
            for _ in range(max_iterations):
                current, jacobian = self.forward(
                    positions, with_jacobian=True
                )
                position_error = target_position - current[:3, 3]
                orientation_error = _rotation_error(
                    current[:3, :3], target_rotation
                )
                ...
                score = np.linalg.norm(position_error) + (
                    0.12 * np.linalg.norm(orientation_error)
                )
                if score < best_score:
                    best_score = score
                    best = positions.copy()
                if (
                    np.linalg.norm(position_error) < 0.004
                    and np.linalg.norm(orientation_error) < 0.12
                ):
                    return positions
        ...
        if best_score < 0.035:
            return best
        raise ValueError(f"IK did not converge; best weighted error={best_score:.4f}")
